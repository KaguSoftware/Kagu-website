...
export async function generateMetadata({
  ...
  const { data } = await supabase
    ...
    .maybeSingle();
  return { title: data ? `Strategy — ${data.url}` : "SEO strategy" };
}
...
export default async function StrategyJobDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
      ...
      {!report || !u ? (
        <p className="border border-dashed border-neutral p-8 text-center text-sm text-slate-ink">
          {job.status === "pending" || job.status === "running"
            ? "The strategy appears here once the worker finishes this job."
        ...
        <>
          ...
          {/* Embedded technical audit — same view as the audit tab. */}
          {report.audit ? (
            ...
            <p className="text-xs text-slate-ink">
              ...
              the agent to run it separately, or queue one from the Site audit
              tab.
            </p>
          )}
        </>
